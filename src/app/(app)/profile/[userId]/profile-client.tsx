...
import {
  ...
  Tag, MessageSquare, CheckCircle2, Eye, Network, ArrowRight,
} from "lucide-react";
          ...
          >
            ...
            <div className="min-w-0 flex-1">
              ...
              <p className="text-xs text-gray-400 truncate">
                {p.title ?? p.company ?? (p.role === "PARTNER" ? "Migration Partner" : "Staky member")}
              </p>
            </div>
          </Link>
        ))}
    ...
    <div className="max-w-4xl mx-auto">
      <div className="grid lg:grid-cols-[1fr_256px] gap-4 items-start">
        ...
        <div className="bg-white rounded-2xl border border-gray-200 overflow-hidden shadow-sm">
          ...
          <div className="px-6 pb-7">
            ...
            <div className="flex items-end justify-between -mt-11 mb-5">
              ...
              <div className="flex items-center gap-2 mb-0.5">
                {isSelf ? (
                  <Link
                    href="/settings"
                    className="inline-flex items-center h-8 px-3.5 rounded-lg border border-gray-200 text-xs font-semibold text-gray-600 bg-white hover:bg-gray-50 transition-colors"
                  >
                    Edit profile
                  </Link>
                ) : (
                    ...
                    {isPartner ? (
                      <>
                        ...
                        {user.partner?.approved && (
                          <Link
                            href="/partners"
                            className="inline-flex items-center gap-1.5 h-8 px-3.5 rounded-lg text-xs font-semibold bg-[#0F6E56] text-white hover:bg-[#0a5a45] transition-colors"
                          >
            ...
            <div className="flex items-center gap-2.5 flex-wrap mb-1">
              <h1 className="text-xl font-bold text-gray-900 tracking-tight">{user.name ?? "Anonymous"}</h1>
              {isPartner && (
                <span className="inline-flex items-center rounded-md bg-blue-50 border border-blue-100 px-2 py-0.5 text-[11px] font-semibold text-[#2A5FA5]">
                  Migration Partner
                </span>
              )}
            </div>
              ...
              {user.partner?.approved && (
                <>
                  ...
                  <span className="flex items-center gap-1">
                    ...
                    {user.partner.rating.toFixed(1)} rating · {user.partner.projectCount} projects
                  </span>
                </>
              )}
            ...
            <div className="flex items-center gap-4 mb-5">
              {isPartner ? (
                ...
                <>
                  <Link href="/network?tab=followers" className="group">
                    <span className="text-sm font-bold text-gray-900 group-hover:text-[#0F6E56] transition-colors">
                      {followerCount}
                    </span>
                  ...
                  <Link href="/network?tab=following" className="group">
                    <span className="text-sm font-bold text-gray-900 group-hover:text-[#0F6E56] transition-colors">
                      {followingCount}
                    </span>
            ...
            {/* Partner specialties */}
            {user.partner?.specialty && user.partner.specialty.length > 0 && (
              <div className="mb-5">
                ...
                <div className="flex flex-wrap gap-1.5">
                  {user.partner.specialty.map((s) => (
                    <span
                      key={s}
                      className="text-xs bg-blue-50 text-[#2A5FA5] border border-blue-100 px-2.5 py-1 rounded-md font-medium"
                    >
                      {s}
                    </span>
                  ))}
            ...
            {/* User interests */}
            {!isPartner && user.interests.length > 0 && (
              <div className="mb-5">
        ...
        <div className="space-y-3">
          ...
          {/* Profile Strength (own) */}
          {isSelf && <ProfileStrength user={user} />}

          {/* Profile views link (own) */}
          {isSelf && (
            <Link href="/profile/views" className="block">
            ...
            <div className="rounded-xl overflow-hidden border border-blue-100 shadow-sm">
              ...
              >
                ...
                <p className="text-xs text-blue-200 leading-relaxed mb-3">
                  {user.name ?? "This partner"} can guide your team through EU software migration.
                </p>
                <Link
                  href="/partners"
                  className="w-full inline-flex items-center justify-center gap-1.5 h-8 bg-white text-[#2A5FA5] text-xs font-semibold rounded-lg hover:bg-blue-50 transition-colors"
                >
