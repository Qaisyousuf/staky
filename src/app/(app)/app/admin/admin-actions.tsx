...
export function RequestActions({
  ...
  requestId: string;
  status: string;
  partners?: { userId: string; companyName: string }[];
}) {
  const router = useRouter();
  const [pending, startTransition] = useTransition();
  const [selectedPartner, setSelectedPartner] = useState("");

  function act(action: "cancel" | "reopen" | "review") {
    startTransition(async () => {
  ...
  function matchPartner() {
    ...
    startTransition(async () => {
      await adminMatchPartner(requestId, selectedPartner);
      router.refresh();
    });
  }
  ...
  const canMatch = ["PENDING", "UNDER_REVIEW"].includes(status) && partners && partners.length > 0;

  return (
    <div className="flex flex-col gap-1.5">
      <div className="flex items-center gap-1 flex-wrap">
        {canReview && (
          <ActionBtn onClick={() => act("review")} disabled={pending} variant="blue">
      ...
      {canMatch && (
        <div className="flex items-center gap-1">
          ...
          >
            ...
            {partners.map((p) => (
              <option key={p.userId} value={p.userId}>{p.companyName}</option>
            ))}
          </select>
          <ActionBtn onClick={matchPartner} disabled={pending || !selectedPartner} variant="green">
